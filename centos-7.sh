...
echo Install wget for easyier scripting
yum install -y wget >> update.log 2>&1

echo Download and unpack maven jdk eclipse chrome - and yum install devel tools and desktop async
...
echo  Add user devel and let sudoers run with no pass
...
echo Create vnc server services for root :1 and for devel user :2
...
echo Maven settings.xml for opendaylight devs
...
echo Add maven options - some might be deprecated
echo 'export MAVEN_OPTS='"'"'-Xmx1048m -XX:MaxPermSize=512m'"'" >> /home/devel/.bashrc

echo Download gitconfig for useful git aliases and setup
wget -q -O - https://cdn.rawgit.com/alexfeigin/devel/master/gitconfig > /etc/gitconfig

echo Disable firewalld and NetworkManager enable legacy network
netsvc()
{
...
netsvc > netsvc.log 2>&1

echo "Setup terminal shortcut (create script, run as autostart app in gnome)"
...
echo 'Fix "Authentication is required to set the network proxy used for downloading packages" message'
echo "X-GNOME-Autostart-enabled=false" >> /etc/xdg/autostart/gnome-software-service.desktop

echo Create an identity file ssh-keygen
